...
// router.post('/', authMiddleware, upload.single('image'), async (req, res,next) => {  //i didn't have authmiddleware now
...
try{ 
   ...
   return  res.status(200).json({ success: true, url: result.secure_url, publicId: result.public_id })
}catch(error){
   next(error)
}
